                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  {(() => {
                    ...
                    return sortedDates.map(dateStr => (
                      <div key={dateStr} style={{ marginBottom: 24, backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border-color)', borderRadius: 12, overflow: 'hidden' }}>
                        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border-color)', backgroundColor: 'rgba(0,0,0,0.2)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <div style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: 8 }}>
                            <Calendar size={16} color="var(--accent-primary)" />
                            Scan Run: {dateStr}
                          </div>
                          <div className="badge badge-outline" style={{ fontSize: '0.7rem' }}>{groups[dateStr].length} Findings</div>
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', padding: 16, gap: 12 }}>
                          {groups[dateStr].map(v => (
                            <div 
                              key={v.id}
