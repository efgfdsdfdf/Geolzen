        ...
        try {
          ...
          // Check if scan completed
            if (entry.type === 'complete') {
              ...
              const passedChecks = [];
              if (result?.metadata?.scanners) {
                const s = result.metadata.scanners;
                if (s.dns && s.dns.findingsCount === 0) passedChecks.push({ name: 'DNS Security', desc: 'SPF, DMARC configured correctly' });
                if (s.headers && s.headers.findingsCount === 0) passedChecks.push({ name: 'Security Headers', desc: 'All critical headers present' });
                if (s.cookies && s.cookies.findingsCount === 0) passedChecks.push({ name: 'Cookie Security', desc: 'HttpOnly, Secure, SameSite enforced' });
                if (s.ssl && s.ssl.findingsCount === 0) passedChecks.push({ name: 'SSL/TLS Cryptography', desc: 'Valid certificate, modern protocols' });
                if (s.dependencies && s.dependencies.findingsCount === 0) passedChecks.push({ name: 'Dependency Analysis', desc: 'No known CVEs in package tree' });
                if (s.cors && s.cors.findingsCount === 0) passedChecks.push({ name: 'CORS Policy', desc: 'Strict origin policies enforced' });
                if (s.ports && s.ports.findingsCount === 0) passedChecks.push({ name: 'Network Port Probing', desc: 'No exposed management interfaces' });
                if (s.cms && s.cms.findingsCount === 0) passedChecks.push({ name: 'CMS & App Layer', desc: 'No admin panels or leaked secrets' });
                if (s.sensitiveFiles && s.sensitiveFiles.findingsCount === 0) passedChecks.push({ name: 'Sensitive Files', desc: 'No exposed .env, .git, or backups' });
                if (s.xss && s.xss.findingsCount === 0) passedChecks.push({ name: 'XSS Payload Testing', desc: 'Input sanitization verified' });
                if (s.sqli && s.sqli.findingsCount === 0) passedChecks.push({ name: 'SQL Injection Testing', desc: 'Queries properly parameterized' });
                if (s.openRedirect && s.openRedirect.findingsCount === 0) passedChecks.push({ name: 'Open Redirect Testing', desc: 'Redirect endpoints secured' });
              }
              ...
              setTargets(prev => prev.map(t => {
                if (t.id === selectedTarget.id) {
                  return { ...t, scanStatus: 'completed', lastScanDate: scanDate, passedChecks };
                }
                return t;
              }));
            }
      ...
      if (t.id === selectedTarget.id) {
        const updatedVulns = t.vulnerabilities.map(v => {
          if (v.id === vulnId) {
            return {
              ...v,
              chatHistory: [...v.chatHistory, { sender: 'user', text: userText }, { sender: 'analyst', text: '', isTyping: true }]
            };
          }
          return v;
        });
        ...
        if (t.id === selectedTarget.id) {
          const updatedVulns = t.vulnerabilities.map(v => {
            if (v.id === vulnId) {
              return {
                ...v,
                chatHistory: [...v.chatHistory.filter(msg => !msg.isTyping), { sender: 'analyst', text: analystResponse }]
              };
            }
            return v;
          });
        ...
        if (t.id === selectedTarget.id) {
          const updatedVulns = t.vulnerabilities.map(v => {
            if (v.id === vulnId) {
              return {
                ...v,
                chatHistory: [...v.chatHistory.filter(msg => !msg.isTyping), { sender: 'analyst', text: `⚠️ System Error: ${err.message}. Please check if the Anthropic API key is properly configured in the backend environment.` }]
              };
            }
            return v;
          });
...
=========================================
            ...
            <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 20 }}>
                <div className="glass-panel" style={{ padding: 28, display: 'flex', flexDirection: 'column' }}>
                  ...
                  <div style={{ fontSize: '2rem', fontWeight: 800, margin: '12px 0' }}>$0<span style={{ fontSize: '0.9rem', color: 'var(--text-muted)' }}>/forever</span></div>
                  <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: 16 }}>Perfect for independent developers to establish a secure foundation.</p>
                  <ul style={{ paddingLeft: 18, fontSize: '0.85rem', color: 'var(--text-secondary)', display: 'flex', flexDirection: 'column', gap: 8, flex: 1, margin: '0 0 20px 0' }}>
                    <li><strong>1 Target Asset</strong></li>
                    <li>DNS Reconnaissance (SPF, DMARC)</li>
                    <li>HTTP Security Header Audit</li>
                ...
                <div className="glass-panel" style={{ padding: 28, display: 'flex', flexDirection: 'column', border: '1px solid var(--accent-orange)' }}>
                  ...
                  <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: 16 }}>Built for startups needing comprehensive cryptographic visibility.</p>
                  <ul style={{ paddingLeft: 18, fontSize: '0.85rem', color: 'var(--text-secondary)', display: 'flex', flexDirection: 'column', gap: 8, flex: 1, margin: '0 0 20px 0' }}>
                    <li><strong>3 Target Assets</strong></li>
                    <li>SSL/TLS Certificate Analysis</li>
                    <li>Dependency Vulnerability Scan (SCA)</li>
                    <li>CORS Misconfiguration Detection</li>
                    <li>Automated Email Security Alerts</li>
                ...
                <div className="glass-panel" style={{ padding: 28, display: 'flex', flexDirection: 'column' }}>
                  ...
                  <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: 16 }}>Enterprise-grade deep-layer active network infrastructure probing.</p>
                  <ul style={{ paddingLeft: 18, fontSize: '0.85rem', color: 'var(--text-secondary)', display: 'flex', flexDirection: 'column', gap: 8, flex: 1, margin: '0 0 20px 0' }}>
                    <li><strong>Unlimited Target Assets</strong></li>
                    <li>Network Port & Sensitive File Probing</li>
                    <li>Deep CMS & App Layer Crawling</li>
                    <li><strong>XSS & SQLi Payload Injection</strong></li>
                    <li><strong>Open Redirect Testing</strong></li>
  ...
  return (
    <div className="app-layout-wrapper" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'row', backgroundColor: 'var(--bg-main)' }}>
      
      {/* Mobile Top Bar */}
      <div className="mobile-top-bar" style={{ justifyContent: 'space-between', padding: '16px 24px', borderBottom: '1px solid var(--border-color)', backgroundColor: 'var(--bg-panel)', alignItems: 'center' }}>
         <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <Shield size={20} color="var(--accent-primary)" />
            <span style={{ fontWeight: 800, fontSize: '1.2rem', letterSpacing: '1px' }}>GEOLZEN</span>
         </div>
         <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} style={{ background: 'transparent', border: 'none', color: 'var(--text-primary)', cursor: 'pointer' }}>
      ...
      <div className={`mobile-nav-overlay ${isMobileMenuOpen ? 'open' : ''}`}>
        <button onClick={() => selectTab('overview')} style={{ width: '100%', textAlign: 'left', padding: '12px 16px', borderRadius: 6, background: activeTab === 'overview' ? 'var(--accent-primary)' : 'transparent', color: activeTab === 'overview' ? '#fff' : 'var(--text-primary)', border: 'none', display: 'flex', alignItems: 'center', gap: 12, fontSize: '1.1rem' }}>
           <Search size={20} /> Overview
        </button>
        <button onClick={() => selectTab('targets')} style={{ width: '100%', textAlign: 'left', padding: '12px 16px', borderRadius: 6, background: activeTab === 'targets' ? 'var(--accent-primary)' : 'transparent', color: activeTab === 'targets' ? '#fff' : 'var(--text-primary)', border: 'none', display: 'flex', alignItems: 'center', gap: 12, fontSize: '1.1rem' }}>
           <Server size={20} /> Targets
        </button>
        <button onClick={() => selectTab('scan')} style={{ width: '100%', textAlign: 'left', padding: '12px 16px', borderRadius: 6, background: activeTab === 'scan' ? 'var(--accent-primary)' : 'transparent', color: activeTab === 'scan' ? '#fff' : 'var(--text-primary)', border: 'none', display: 'flex', alignItems: 'center', gap: 12, fontSize: '1.1rem' }}>
           <Terminal size={20} /> Scans
        </button>
        <button onClick={() => selectTab('findings')} style={{ width: '100%', textAlign: 'left', padding: '12px 16px', borderRadius: 6, background: activeTab === 'findings' ? 'var(--accent-primary)' : 'transparent', color: activeTab === 'findings' ? '#fff' : 'var(--text-primary)', border: 'none', display: 'flex', alignItems: 'center', gap: 12, fontSize: '1.1rem' }}>
           <ShieldAlert size={20} /> Findings
        </button>
        <button onClick={() => selectTab('settings')} style={{ width: '100%', textAlign: 'left', padding: '12px 16px', borderRadius: 6, background: activeTab === 'settings' ? 'var(--accent-primary)' : 'transparent', color: activeTab === 'settings' ? '#fff' : 'var(--text-primary)', border: 'none', display: 'flex', alignItems: 'center', gap: 12, fontSize: '1.1rem' }}>
        ...
        <button 
           ...
           style={{ width: '100%', textAlign: 'left', padding: '12px 16px', background: 'transparent', color: 'var(--color-critical)', border: 'none', display: 'flex', alignItems: 'center', gap: 12, fontSize: '1.1rem', marginTop: 'auto' }}
        >
           <X size={20} /> Logout
        </button>
      </div>
      ...
      {/* Global Strip */}
      <div className="desktop-sidebar" style={{ width: 64, backgroundColor: 'var(--bg-strip)', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '16px 0', borderRight: '1px solid var(--border-color)', zIndex: 10, flexShrink: 0 }}>
         <div style={{ width: 32, height: 32, borderRadius: 8, background: 'var(--accent-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 32 }}>
           <Shield size={20} color="#fff" />
         </div>
         <div style={{ display: 'flex', flexDirection: 'column', gap: 24, alignItems: 'center' }}>
           <Server size={20} color="var(--accent-primary)" style={{ cursor: 'pointer' }} />
           <GithubIcon size={20} color="var(--text-secondary)" style={{ cursor: 'pointer' }} />
           <Plus size={20} color="var(--text-secondary)" style={{ cursor: 'pointer' }} />
         </div>
      </div>

      {/* Secondary Sidebar */}
      <div className="desktop-sidebar" style={{ width: 250, backgroundColor: 'var(--bg-panel)', display: 'flex', flexDirection: 'column', borderRight: '1px solid var(--border-color)', zIndex: 9, height: '100vh', overflowY: 'auto', flexShrink: 0 }}>
         
         {/* Workspace Selector */}
         <div onClick={() => alert('Workspace switching coming soon!')} style={{ padding: '16px 20px', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
            <div style={{ width: 24, height: 24, borderRadius: '50%', backgroundColor: '#6b21a8', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.8rem', fontWeight: 600 }}>M</div>
            <span style={{ fontWeight: 600, fontSize: '0.9rem', color: 'var(--text-primary)' }}>My Workspace</span>
            <ChevronDown size={14} color="var(--text-secondary)" style={{ marginLeft: 'auto' }} />
         </div>

         {/* Menu Items */}
         <div style={{ padding: '24px 12px' }}>
            <button onClick={() => alert('Feature coming soon!')} style={{ width: '100%', textAlign: 'left', padding: '8px 12px', background: 'transparent', color: 'var(--text-secondary)', border: 'none', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', fontSize: '0.9rem', marginBottom: 24 }}>
               ← Environment
            </button>
            
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '0 12px', marginBottom: 32 }}>
               <Globe size={18} color="var(--text-primary)" />
               <span style={{ fontSize: '1.1rem', fontWeight: 600, color: 'var(--text-primary)' }}>Geolzen</span>
            </div>

            <div style={{ padding: '0 12px', fontSize: '0.75rem', fontWeight: 600, color: 'var(--text-muted)', letterSpacing: '0.05em', marginBottom: 12, textTransform: 'uppercase' }}>Monitor</div>
            
            <button onClick={() => selectTab('overview')} style={{ width: '100%', textAlign: 'left', padding: '8px 12px', borderRadius: 6, background: activeTab === 'overview' ? 'var(--accent-primary)' : 'transparent', color: activeTab === 'overview' ? '#fff' : 'var(--text-secondary)', border: 'none', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', fontSize: '0.9rem', marginBottom: 4 }}>
               <Search size={16} /> Overview
            </button>
            <button onClick={() => selectTab('targets')} style={{ width: '100%', textAlign: 'left', padding: '8px 12px', borderRadius: 6, background: activeTab === 'targets' ? 'var(--accent-primary)' : 'transparent', color: activeTab === 'targets' ? '#fff' : 'var(--text-secondary)', border: 'none', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', fontSize: '0.9rem', marginBottom: 4 }}>
               <Server size={16} /> Targets
            </button>

            <div style={{ padding: '0 12px', fontSize: '0.75rem', fontWeight: 600, color: 'var(--text-muted)', letterSpacing: '0.05em', marginTop: 24, marginBottom: 12, textTransform: 'uppercase' }}>Manage</div>

            <button onClick={() => selectTab('scan')} style={{ width: '100%', textAlign: 'left', padding: '8px 12px', borderRadius: 6, background: activeTab === 'scan' ? 'var(--accent-primary)' : 'transparent', color: activeTab === 'scan' ? '#fff' : 'var(--text-secondary)', border: 'none', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', fontSize: '0.9rem', marginBottom: 4 }}>
               <Terminal size={16} /> Scans
            </button>
            <button onClick={() => selectTab('findings')} style={{ width: '100%', textAlign: 'left', padding: '8px 12px', borderRadius: 6, background: activeTab === 'findings' ? 'var(--accent-primary)' : 'transparent', color: activeTab === 'findings' ? '#fff' : 'var(--text-secondary)', border: 'none', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', fontSize: '0.9rem', marginBottom: 4 }}>
               <ShieldAlert size={16} /> Findings
            </button>
            <button onClick={() => selectTab('settings')} style={{ width: '100%', textAlign: 'left', padding: '8px 12px', borderRadius: 6, background: activeTab === 'settings' ? 'var(--accent-primary)' : 'transparent', color: activeTab === 'settings' ? '#fff' : 'var(--text-secondary)', border: 'none', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', fontSize: '0.9rem', marginBottom: 4 }}>
               <SettingsIcon size={16} /> Settings
            </button>

            <div style={{ marginTop: 'auto', borderTop: '1px solid var(--border-color)', paddingTop: 16, marginTop: 32 }}>
               <button 
                 onClick={() => { localStorage.removeItem('geolzen_auth'); setIsAuthenticated(false); setAuthMessage(null); setPrivacyAgreed(false); }}
                 style={{ width: '100%', textAlign: 'left', padding: '8px 12px', background: 'transparent', color: 'var(--text-secondary)', border: 'none', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', fontSize: '0.85rem' }}
               >
                 <X size={16} /> Logout
               </button>
            </div>
      ...
      <div style={{ flex: 1, height: '100vh', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ padding: '16px 32px', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexShrink: 0, backgroundColor: 'var(--bg-main)' }}>
             <div style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: '0.9rem', color: 'var(--text-secondary)' }}>
                <GithubIcon size={16} /> efgfdsdfdf / Geolzen <span style={{ margin: '0 8px' }}>/</span> <GitPullRequest size={16} /> main
                <a href="https://geolzen.vercel.app" target="_blank" rel="noreferrer" style={{ marginLeft: 16, color: 'var(--accent-primary)', display: 'flex', alignItems: 'center', gap: 4 }}>
                  https://geolzen.vercel.app <ExternalLink size={12} />
                </a>
             </div>
          </div>

          {/* Main Content Container */}
          <div className="main-content-area" style={{ padding: '32px', maxWidth: '1200px', width: '100%', margin: '0 auto' }}>
          ...
          <div>
            ...
            {selectedTarget?.vulnerabilities.length === 0 ? (
              <div className="glass-panel" style={{ padding: 60, textAlign: 'center', color: 'var(--text-secondary)' }}>
                ...
                <p style={{ marginBottom: 32 }}>Ensure you have verified the target ownership, signed the consent agreement, and run a full active scan.</p>
                
                {selectedTarget?.passedChecks?.length > 0 && (
                  <div style={{ textAlign: 'left', maxWidth: '600px', margin: '0 auto', backgroundColor: 'rgba(16, 17, 24, 0.4)', borderRadius: 8, padding: 24, border: '1px solid rgba(255, 255, 255, 0.05)' }}>
                    <h4 style={{ color: 'var(--color-success)', margin: '0 0 16px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
                      <CheckCircle size={18} /> Verified Secure Systems
                    </h4>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                      {selectedTarget.passedChecks.map((check, i) => (
                        <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                          <Shield size={14} color="var(--color-success)" style={{ marginTop: 2, flexShrink: 0 }} />
                          <div>
                            <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-primary)' }}>{check.name}</div>
              ...
              <div className="grid-layout-findings">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  {/* List cards grouped by scan date */}
                  {(() => {
                    const filteredVulns = selectedTarget?.vulnerabilities.filter(v => vulnFilter === 'all' || v.severity === vulnFilter) || [];
                    if (filteredVulns.length === 0) return null;
                    
                    const groups = {};
                    filteredVulns.forEach(v => {
                      const dateStr = v.createdAt ? new Date(v.createdAt).toLocaleDateString(undefined, { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' }) : 'Unknown Scan Date';
                          ...
                          {sortedDates.map(dateStr => (
                            ...
                            >
                              <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                                ...
                                <div>
                                  <div style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--text-primary)', marginBottom: 4 }}>Scan Run: {dateStr}</div>
                                  <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>View security findings and reports</div>
                                </div>
                              </div>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                                <div className="badge badge-outline" style={{ fontSize: '0.8rem', padding: '6px 12px' }}>{groups[dateStr].length} Findings</div>
                    ...
                    const currentGroup = groups[selectedScanDate] || [];
                    ...
                    return (
                      <div>
                        ...
                        >
                          <ChevronRight size={16} style={{ transform: 'rotate(180deg)' }} /> Back to Scan History
                        </button>
                        
                        <div style={{ marginBottom: 24, paddingBottom: 20, borderBottom: '1px solid var(--border-color)' }}>
                          <h2 style={{ fontSize: '1.4rem', fontWeight: 700, margin: '0 0 8px 0' }}>Scan of {selectedScanDate}</h2>
                          <p style={{ color: 'var(--text-secondary)', margin: 0, fontSize: '0.9rem' }}>Reviewing {currentGroup.length} findings from this security run.</p>
                        </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                          {currentGroup.map(v => (
                            <div 
                              ...
                              style={{
                                padding: 16,
                                backgroundColor: selectedVulnId === v.id ? 'rgba(255, 107, 0, 0.08)' : 'rgba(255, 255, 255, 0.02)',
                ...
                {/* 3rd Pane: Copilot Sticky Sidebar (Desktop only) */}
                <div className="copilot-panel-desktop">
                  <div className="analyst-chat-panel" style={{ height: '100%' }}>
                    ...
                    <div className="analyst-chat-body">
                      {selectedVuln ? selectedVuln.chatHistory.map((chat, idx) => (
                        ...
                        <div style={{ textAlign: 'center', color: 'var(--text-secondary)', padding: '20px 0', fontSize: '0.85rem' }}>
                          Select a finding to chat with the analyst.
                        </div>
                      )}
                    </div>
                    ...
                    <form onSubmit={handleChatMessageSubmit} className="chat-input-area">
                      <input 
                        type="text" 
                        placeholder="Ask details (e.g. 'Is this a false positive?')"
                        value={chatMessageInput}
                        onChange={(e) => setChatMessageInput(e.target.value)}
                        disabled={!selectedVuln}
                      />
                      <button type="submit" disabled={!selectedVuln}>
                        <Send size={16} />
                      </button>
                    </form>
                  </div>
            ...
            {/* Show passed checks even if there are vulnerabilities */}
            {selectedTarget?.vulnerabilities?.length > 0 && selectedTarget?.passedChecks?.length > 0 && (
              <div style={{ marginTop: 32 }} className="glass-panel">
                <div style={{ padding: 24 }}>
                  <h4 style={{ color: 'var(--color-success)', margin: '0 0 16px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
                    <CheckCircle size={18} /> Verified Secure Systems
                  </h4>
                  <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', marginBottom: 20 }}>
                    The following scan modules ran and found <strong>0</strong> vulnerabilities, confirming these parts of your infrastructure are secure.
                  </p>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: 16 }}>
                    {selectedTarget.passedChecks.map((check, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: 12, backgroundColor: 'rgba(255,255,255,0.02)', borderRadius: 6, border: '1px solid rgba(255,255,255,0.03)' }}>
                        <Shield size={16} color="var(--color-success)" style={{ marginTop: 2, flexShrink: 0 }} />
                        <div>
                          <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-primary)' }}>{check.name}</div>
                          <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{check.desc}</div>
                        </div>
...
{'}'}
      ...
      {showPrivacyModal && (
        <div style={{
          ...
          <div className="glass-panel" style={{ width: '550px', maxHeight: '80vh', display: 'flex', flexDirection: 'column', padding: 28 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
              <button onClick={() => setShowPrivacyModal(false)} className="glow-btn">
                Close Policy
              </button>
              </div>
            </div>
          ...
          <div className="glass-panel" style={{ width: '100%', maxWidth: 800, padding: 32, position: 'relative' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 24 }}>
              <div className="glass-panel" style={{ padding: 24, display: 'flex', flexDirection: 'column', border: '1px solid var(--accent-orange)' }}>
                ...
                <ul style={{ paddingLeft: 18, fontSize: '0.85rem', color: 'var(--text-secondary)', display: 'flex', flexDirection: 'column', gap: 8, flex: 1, margin: '0 0 20px 0' }}>
                  <li><strong>3 Target Assets</strong></li>
                  <li>Automated Dependency Audits (SCA)</li>
                  <li>SSL/TLS Cryptographic Analysis</li>
                  <li>Automated Email Security Alerts</li>
                </ul>
              ...
              <div className="glass-panel" style={{ padding: 24, display: 'flex', flexDirection: 'column' }}>
                ...
                <ul style={{ paddingLeft: 18, fontSize: '0.85rem', color: 'var(--text-secondary)', display: 'flex', flexDirection: 'column', gap: 8, flex: 1, margin: '0 0 20px 0' }}>
                  <li><strong>Unlimited Target Assets</strong></li>
                  <li>Active Network Port Probing</li>
                  <li>Deep App-Layer CMS Scanning</li>
                  <li>Auto-Generated Firewall Rules</li>
                </ul>
      ...
      {/* Mobile Copilot Floating Button & Drawer */}
      {activeTab === 'findings' && (
        <>
          <button 
            className="floating-copilot-btn" 
            onClick={() => setShowMobileCopilot(true)}
            aria-label="Open Security Analyst Copilot"
          >
            <MessageSquare size={24} />
          </button>

          <div className={`copilot-drawer-overlay ${showMobileCopilot ? 'open' : ''}`} onClick={() => setShowMobileCopilot(false)}>
            <div className="copilot-drawer" onClick={(e) => e.stopPropagation()}>
              <button 
                onClick={() => setShowMobileCopilot(false)}
                style={{ position: 'absolute', top: 16, right: 16, background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', zIndex: 10 }}
              >
                <X size={24} />
              </button>
              
              <div className="analyst-chat-panel" style={{ height: '100%', border: 'none', borderRadius: 0, backgroundColor: 'transparent' }}>
                <div className="analyst-chat-header" style={{ paddingRight: 48, borderTopLeftRadius: 20, borderTopRightRadius: 20 }}>
                  <MessageSquare size={16} color="var(--accent-orange)" />
                  <span style={{ fontSize: '0.85rem', fontWeight: 600 }}>Security Analyst Copilot</span>
                  <div style={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: 'var(--color-success)', marginLeft: 'auto' }}></div>
                  <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Online</span>
                ...
                <div className="analyst-chat-body" style={{ paddingBottom: 0 }}>
                  {selectedVuln ? selectedVuln.chatHistory.map((chat, idx) => (
                    <div key={idx} className={`chat-bubble ${chat.sender === 'user' ? 'chat-bubble-user' : 'chat-bubble-analyst'}`}>
                      {chat.isTyping ? (
                        <div className="typing-indicator">
                ...
                <form onSubmit={handleChatMessageSubmit} className="chat-input-area" style={{ borderBottomLeftRadius: 0, borderBottomRightRadius: 0, paddingBottom: 24 }}>
                  <input 
                    type="text" 
                    placeholder="Ask details (e.g. 'Is this a false positive?')"
                    value={chatMessageInput}
