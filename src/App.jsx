...
export default function App() {
  ...
  const handleAddTarget = async (e) => {
    ...
    if (targets.length >= maxTargets) {
      alert(`You have reached the target limit for your ${orgData?.plan_tier || 'free'} plan. Please upgrade to add more targets.`);
      return;
    }
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                <div style={{ padding: '8px 16px', background: 'rgba(255, 107, 0, 0.1)', border: '1px solid var(--accent-orange)', borderRadius: 8, display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  {orgData?.plan_tier !== 'team' && (
                    <button onClick={() => setShowUpgradeModal(true)} className="glow-btn" style={{ padding: '6px 12px', fontSize: '0.8rem' }}>Upgrade</button>
                  )}
                </div>
      ...
      {showUpgradeModal && (
        <div style={{
          ...
          <div className="glass-panel" style={{ width: '100%', maxWidth: 800, padding: 32, position: 'relative' }}>
            ...
            <h2 style={{ margin: '0 0 8px 0', fontSize: '1.8rem' }}>Upgrade Your Plan</h2>
            <p style={{ color: 'var(--text-secondary)', margin: '0 0 24px 0' }}>Unlock more targets and advanced autonomous security capabilities.</p>
