...
const INITIAL_TARGETS = [
  {
    id: 'target-1',
    name: 'geolzen-prod-auth',
    type: 'repository',
    verified: true,
    verificationMethod: 'oauth',
    verificationToken: 'gz_auth_token_oauth_3821',
    signedROE: true,
    signedDate: '2026-07-08 10:14',
    scanStatus: 'completed',
    scanType: 'active',
    lastScanDate: '2026-07-08 10:20',
    vulnerabilitiesCount: { critical: 1, high: 2, medium: 1, low: 0 },
    vulnerabilities: [
      {
        id: 'vuln-1',
        title: 'Prototype Pollution in lodash (CVE-2020-8203)',
        severity: 'critical',
        category: 'Dependency SCA',
        description: 'Lodash versions prior to 4.17.21 contain a prototype pollution vulnerability. An attacker can inject properties into Object.prototype through JSON payloads, which can lead to Remote Code Execution (RCE) or Application Crash (DoS) depending on application logic.',
        impact: 'Critical. Allows remote attackers to corrupt Node.js core variables, hijack runtime process flow, or execute code context-wide.',
        solution: 'Upgrade lodash dependency in package.json to version 4.17.21 or higher.',
        remediationType: 'pr',
        remediated: false,
    ...
    "pg": "^8.11.0"
        ...
        chatHistory: [
          { sender: 'analyst', text: "Hi, I am your Geolzen Security Analyst copilot. I found that Express is loading lodash 4.17.15 which has an active CVE. Let me know if you want to know how we exploit it, or click 'Generate Pull Request' above to fix." }
        ]
      },
      {
        id: 'vuln-2',
        title: 'Publicly Accessible AWS S3 Bucket Policy (CIS 2.1)',
        severity: 'high',
        category: 'Cloud Posture',
        description: 'The bucket policy for "geolzen-static-assets" allows wildcard read access (*). Any user on the public internet can read or download objects stored in this bucket without authentication.',
        impact: 'High. Potential exposure of sensitive documents, backup dumps, source code configs, or private assets.',
        solution: 'Modify the AWS S3 Bucket policy to restrict access to authenticated IAM roles or a specific CloudFront Origin Access Identity (OAI).',
  ...
  "Statement": [
    {
      ...
      "Principal": "*",
      "Action": "s3:GetObject",
      "Resource": "arn:aws:s3:::geolzen-static-assets/*"
    }
  ]
}`,
        fixedCode: `{
  ...
  "Statement": [
    {
      "Sid": "PrivateReadWithOAI",
      "Effect": "Allow",
      "Principal": {
        "AWS": "arn:aws:iam::cloudfront:user/CloudFront-Origin-Access-Identity-E2XXXXXXXXX"
      },
      "Action": "s3:GetObject",
      "Resource": "arn:aws:s3:::geolzen-static-assets/*"
    }
        ...
        chatHistory: [
          { sender: 'analyst', text: "Hello! Our AWS integration detected a public bucket policy. S3 buckets should never be readable by wildcard asterisks in production. How would you like me to help?" }
        ]
      },
      {
        id: 'vuln-3',
        ...
        severity: 'high',
        category: 'DAST Audit',
        description: 'The application is missing a Content Security Policy header. A CSP header tells the browser which dynamic resources are allowed to load, preventing Cross-Site Scripting (XSS) and clickjacking injection.',
        impact: 'High. Increases vulnerability to Client-Side Injection, Session Hijacking, and malicious script execution in the user browser context.',
        solution: 'Configure your web server (e.g., Nginx, Apache) to return the Content-Security-Policy header.',
        remediationType: 'config',
        remediated: false,
        fileName: 'nginx.conf',
        originalCode: `server {
    listen 443 ssl;
    server_name auth.geolzen.io;
    ...
    add_header Content-Security-Policy "default-src 'self'; script-src 'self' 'unsafe-inline' https://apis.google.com; style-src 'self' 'unsafe-inline'; img-src 'self' data:; frame-ancestors 'none';" always;
    ...
    location / {
        ...
        chatHistory: [
          { sender: 'analyst', text: "This header protects your web portal users from Malicious inline scripts. I can provide the header code for Apache or Nginx. What web server are you running?" }
        ]
      },
      {
        id: 'vuln-4',
        title: 'Deprecated TLS 1.0 & TLS 1.1 Protocols Enabled',
        severity: 'medium',
        category: 'Network Recon',
        description: 'The SSL/TLS configuration allows client connections using TLS 1.0 and TLS 1.1 protocols. These older cryptographic protocols suffer from key derivation vulnerabilities and lack forward secrecy.',
        impact: 'Medium. Attackers capable of intercepting traffic (man-in-the-middle) can perform cryptographic downgrade attacks to decrypt secure communications.',
        solution: 'Update the server TLS configurations to only support TLS 1.2 and TLS 1.3.',
        remediationType: 'config',
        remediated: false,
        fileName: 'nginx.conf',
...
ssl_ciphers ECDHE-ECDSA-AES128-GCM-SHA256:ECDHE-RSA-AES128-GCM-SHA256:ECDHE-ECDSA-AES256-GCM-SHA384:ECDHE-RSA-AES256-GCM-SHA384;`,
        chatHistory: [
          { sender: 'analyst', text: "Hi, TLS 1.0/1.1 are officially deprecated. They contain vulnerabilities like BEAST and POODLE. We should enforce TLS 1.2 and TLS 1.3." }
        ]
      }
    ]
  },
  {
    ...
    verificationToken: 'gz-verification-token=gz_domain_440292_txt',
    signedROE: false,
    signedDate: null,
    scanStatus: 'idle',
    scanType: null,
            ...
            <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 20 }}>
                ...
                <div className="glass-panel" style={{ padding: 28, display: 'flex', flexDirection: 'column', border: '1px solid var(--accent-orange)' }}>
                  ...
                  <button onClick={() => { setAuthMode('signup'); setShowAuthGate(true); }} className="glow-btn" style={{ width: '100%' }}>Choose Starter</button>
                </div>
                ...
                <div className="glass-panel" style={{ padding: 28, display: 'flex', flexDirection: 'column' }}>
                  ...
                  <button onClick={() => { setAuthMode('signup'); setShowAuthGate(true); }} className="glow-btn-outline" style={{ width: '100%' }}>Choose Team</button>
                </div>
              </div>
            </div>
          </section>
          ...
          <form onSubmit={handleAuthSubmit}>
            ...
            {authMode === 'signup' && (
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginTop: 16 }}>
                ...
                />
                <label htmlFor="signup-privacy-check" style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', cursor: 'pointer' }}>
                  I agree to the Geolzen{' '}
                  <button 
                    type="button" 
                    onClick={() => setShowPrivacyModal(true)}
                    style={{ background: 'transparent', border: 'none', padding: 0, textDecoration: 'underline', color: 'var(--accent-orange)', cursor: 'pointer', display: 'inline' }}
                  >
                    Privacy Policy
                  </button>{' '}
                  and consent to automated assessments.
                </label>
              </div>
            )}
            ...
            >
              {authLoading ? <RefreshCw size={16} className="animate-spin" /> : authMode === 'login' ? 'Sign In to Console' : 'Initialize Account'}
            </button>
          </form>

          {/* Status indication badge to show database connection mode */}
          <div style={{ marginTop: 16, fontSize: '0.75rem', color: 'var(--text-muted)', textAlign: 'center' }}>
            Connection Mode:{' '}
            <span style={{ color: supabaseClient ? 'var(--color-success)' : 'var(--color-high)', fontWeight: 'bold' }}>
              {supabaseClient ? 'Supabase Live API' : 'Sandbox Simulator Fallback'}
            </span>
          </div>
      ...
      <header style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
            ...
            <button 
              onClick={() => { setIsAuthenticated(false); setAuthMessage(null); setPrivacyAgreed(false); }}
        ...
        <div className={`mobile-nav-overlay ${isMobileMenuOpen ? 'open' : ''}`}>
          ...
          <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: 12, marginTop: 4 }}>
            <button 
              onClick={() => { setIsAuthenticated(false); setAuthMessage(null); setPrivacyAgreed(false); setIsMobileMenuOpen(false); }}
        ...
        {activeTab === 'targets' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32, flexWrap: 'wrap', gap: 16 }}>
              ...
              <button onClick={() => setShowAddModal(true)} className="glow-btn">
          ...
          <div>
            ...
            <div className="grid-layout-settings">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                <button
                  type="button"
                  onClick={() => setSettingsView('supabase')}
                  className={`glow-btn-outline ${settingsView === 'supabase' ? 'active' : ''}`}
                  style={{
                    justifyContent: 'flex-start',
                    border: settingsView === 'supabase' ? '1px solid var(--accent-orange)' : '1px solid transparent',
                    background: settingsView === 'supabase' ? 'rgba(255, 107, 0, 0.1)' : 'transparent',
                    color: settingsView === 'supabase' ? 'var(--accent-orange)' : 'var(--text-secondary)'
                  }}
                >
                  <Database size={16} /> Supabase Sync
                </button>
              </div>
              ...
              <div className="glass-panel">
                ...
                {/* SUPABASE VIEW */}
                {settingsView === 'supabase' && (
                  <div>
                    <h3 style={{ margin: '0 0 20px 0', borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: 10 }}>Supabase Synchronization Portal</h3>
                    
                    <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', lineHeight: 1.5, marginBottom: 20 }}>
                      Geolzen utilizes relational databases to store user metadata securely. Connect your target database instance by supplying credentials:
                    </p>

                    <form onSubmit={handleSupabaseConnect} style={{ marginBottom: 28 }}>
                      <div className="form-group" style={{ marginBottom: 14 }}>
                        ...
                        <input 
                          ...
                          style={{ width: '100%', padding: 10, borderRadius: 6, border: '1px solid rgba(255,255,255,0.08)', backgroundColor: '#040407', color: '#fff', fontSize: '0.85rem', fontFamily: 'var(--font-mono)' }}
                        />
                      </div>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
                        <div style={{ fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: 6 }}>
                          <span style={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: supabaseStatus === 'connected' ? 'var(--color-success)' : supabaseStatus === 'connecting' ? 'var(--color-medium)' : 'var(--text-muted)' }}></span>
                          <span style={{ color: 'var(--text-secondary)', textTransform: 'capitalize' }}>Status: {supabaseStatus}</span>
                        </div>
                        <button type="submit" className="glow-btn" style={{ padding: '8px 16px', fontSize: '0.85rem' }}>
                          {supabaseStatus === 'connecting' ? <RefreshCw size={14} className="animate-spin" /> : 'Connect Database'}
                        </button>
                      </div>
                    </form>

                    <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: 20 }}>
                      <h4 style={{ margin: '0 0 10px 0', fontSize: '0.9rem', color: 'var(--text-primary)' }}>Javascript SDK Initialization</h4>
                      <pre style={{
                        padding: 14,
                        backgroundColor: '#040407',
                        border: '1px solid rgba(255,255,255,0.05)',
...
const supabaseAnonKey = '${supabaseAnonKey.substring(0, 20)}...'

// Initializing Geolzen secure connection client
            ...
            {verificationMethod === 'oauth' && (
              <div style={{ marginBottom: 20, fontSize: '0.85rem' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', padding: '12px', border: '1px dashed rgba(255, 107, 0, 0.2)', borderRadius: 8 }}>
                  <span style={{ color: 'var(--accent-orange)', display: 'flex', alignItems: 'center', gap: 8 }}>
